...
//Sprawdzanie sytuacji specjalnych - przelom roku/miesiaca
	...
	// Funkcje - linki
		...
		function zmien(cos)	{ 
			if(cos=="zwroty") window.location.href = "panel_zmienZW.php";
		}
		...
		function zakoncz()			{
			window.location.href="panel_zakoncz.php?tt="+tt+"&rrr="+rrr+"&pocz="+poczatekTygodnia+"&koniec="+koniecTygodnia+"&baza=bazamgn";
			}

			
		function znajdzParafie(str){
			if(str=="")	{
				document.getElementById("spis").innerHTML= "";
				return;
				}	
			else		{
					...
					xmlhttp.onreadystatechange = function ()	{
							if(this.readyState == 4 && this.status ==200)	{
									document.getElementById("spis").innerHTML = this.responseText ;
							}
						};
					
					xmlhttp.open("GET","user_znajdz.php?dekanat="+str,true);
					xmlhttp.send();
		...
		function znajdzZW(parafia){
					document.getElementById("TESTING_IN_PROGRESS").innerHTML= parafia;
				if(parafia==" ")	{
